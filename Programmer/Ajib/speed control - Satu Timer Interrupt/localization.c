...
extern int ballX = 180;
extern uint8_t ballFound;
extern float movY;
extern float movX;
extern int movCompass;
extern int proxy13 = 0;
extern int proxy14;
extern int ballKeeper;
int counterDelay;
int rotateCount;
int icount=1;
int flagKickoff=0;
...
int prevGameState;
	if(refree == ON ){
		if (gameState == 's') {
			...
			flagKickoff=0;
			if(ballYCoor>BALLGET || proxy13 == 1){
				getBallNew();
				if(ballYCoor<30){
					handleRotateIn();
				}
...
void printData(){
	...
	xCoor = movX;
	yCoor = movY;
	sprintf(lcd,"H:%d Y:%d X:%d ",compassHeading,ballYCoor,ballXCoor);//ballXCoor);
	lcd_putsf(0,0,lcd);
	sprintf(lcd, "G:%c P:%d ",gameState,proxy13);
	lcd_putsf(0,1,lcd);
}
...
int getProxy()
{
	...
	else if ((proxy13 == 0 || proxy14 == 0 ) && keeperFlag == 0)
	{
		return 0;
	}
...
//Magenta
//#define SUDUTGAWANG 279
//Cyan
#define SUDUTGAWANG 65
...
//		rotateAntiClockWise(PID(compassHeading,0,100));
//		rotateAntiClockWise(100);
		if(compassHeading<350)
		{
			rotateClockWise(50);
		}
		else if(compassHeading>50)
		{
			rotateAntiClockWise(50);
		}
//		else if(compassHeading<350)
//		{
//			rotateClockWise(30);
//		}
//		else if(compassHeading>50)
...
int gotoHeadBack(void){
	 if(compassHeading > 350 || compassHeading < 10 || flagBackward == 1){
		if(flagBackward == 0)
		{
			if(ballX == 250)
			{
