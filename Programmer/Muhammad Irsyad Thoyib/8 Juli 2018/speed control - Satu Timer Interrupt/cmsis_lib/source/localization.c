...
//			prevGameState = 's';
			if(dapetbola == 0)
			{
				if(/*ballYCoor>30 || (ballXCoor <= 75 || ballXCoor >= 110)*/ x == 0)
				{
					getBallNew();
					dapetbola = 0;
					if(ballYCoor<=50){
...
//		wasit = 0;
		if(dapetbola == 0)
		{
			if(/*ballYCoor>30 || (ballXCoor <= 75 || ballXCoor >= 110)*/ x == 1)
			{
				getBallNew();
				dapetbola = 0;
				if(ballYCoor<=50){
...
//		{
//			mundur(80);
//			for(strategyCount = 0; strategyCount < 40; strategyCount++)
//			{
//				Delayms(100);
//				if(ballFound == 1)
//				{
...
//		if(compassHeading>180)
//		{
//			rotateAntiClockWise(PID(compassHeading,0,20));
//		}
//		else
			rotateClockWise(PID(compassHeading,0,20));
		handleRotateIn();
		return false;
	 }
}
