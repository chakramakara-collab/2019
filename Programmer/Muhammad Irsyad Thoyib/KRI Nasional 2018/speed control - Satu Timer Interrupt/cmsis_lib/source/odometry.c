...
//			theta -= (float)((int)(theta/TWOPI))*TWOPI;
	...
	// displacement to position with encoders heading
			X_pos += LRmeters * sin(theta) + FBmeters * cos(theta) / CLICKS_PER_METER;
			Y_pos += LRmeters * cos(theta) + FBmeters * (-sin(theta)) / CLICKS_PER_METER;

	// displacement to position with compass heading
			X_poscmps += LRmeters * sin(compassHeading*PI/180) + (FBmeters * cos(compassHeading*PI/180)) / CLICKS_PER_METER;
			Y_poscmps += LRmeters * cos(compassHeading*PI/180) + (FBmeters * (-sin(compassHeading*PI/180)) / CLICKS_PER_METER);
}
